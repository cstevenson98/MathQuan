...
MESolve[rhoinit_, \[ScriptCapitalL]_, {tmin_, tmax_}] := Module[
   {
    eqns, init, soln,
    state = 
     Array[Subscript[rho, ##][t] &, {Length[rhoinit], 
        Length[rhoinit]}] // Vectorise
    },
   eqns = Thread[D[state, t] == \[ScriptCapitalL] . state];
   init = Thread[(state /. {t -> tmin}) == Vectorise[rhoinit]];
   soln = 
    toHilbert[state] /. 
     NDSolve[{eqns, init}, state, {t, tmin, tmax}][[1]]
   ];
...
(* ::Input::Initialization:: *)
DualityRhoSS[n_,g_,\[CapitalDelta]_,Jpm_,method_]:=Table[\[Rho]ss[n,gpm g,\[CapitalDelta],J[#,\[Omega]0pm]&,method],{gpm,{-1,1}},{\[Omega]0pm,{-1,1}}]


(* ::Input::Initialization:: *)
HrMinSec[seconds_]:=Block[
{hrs=Floor[seconds/60.^2],mins,secs},
mins=Floor[(seconds-hrs*60.^2)/60.];
secs=(seconds-hrs*60.^2-mins*60.);
{hrs,mins,secs}];

HrMinSecShow[seconds_]:=StringJoin@(ToString/@({#[[1]]," h ",#[[2]]," m ",#[[3]]," s"})&@HrMinSec[seconds]);
